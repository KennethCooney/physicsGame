(function($, window){ //IIFE
	...
	var wallRight = Bodies.rectangle(655, 482, 100, 960, {isStatic:true}); // x, y , width, height, options
	...
	var backAndForth = Bodies.rectangle(0, 0, 20, 40, {isStatic: true, render: {
					fillStyle: '#ffffff', 
					strokeStyle: '#ffffff', 
					lineWidth: 0
						}}); 
	...
	World.add(engine.world, [ backAndForth, ground, wallRight, wallLeft, spawner, colliderTop, collider100, collider50Left, collider50Right, spinnerA, spinnerB, spinnerC, spinnerD, spinnerBigL, spinnerBigR, spinnerBigC]);
	// run the engine
	Engine.run(engine); 
	// run the renderer
	Render.run(render);
	...
	function animateSpawner(){
		Matter.Events.on(engine, 'beforeUpdate', function() {
			...
			Matter.Body.setPosition(backAndForth, {x: 300 + 240 * Math.cos(engine.timing.timestamp * 0.0005),y:400}); // x + width * speed
	...
	function clickListener(){
		...
		$('canvas').on('click', function(e){
			...
			if (ballCounter === 0){
				...
				Matter.Composite.add(engine.world, [ball]); // add to world
			...
			if (ballCounter === 1){  // indicate to user that you only have 1 ball left
				console.log('is 1')
